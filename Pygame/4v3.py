...
def main_menu():
    global start_button, credits_button, exit_button
    ...
    while True:
        ...
        draw_text('Jogos', small_font, BLACK, screen, screen_width // 2, 225)
...
def menu_jogos():
    botao_jogo1 = pygame.Rect(300, 200, 200, 50)
    botao_jogo2 = pygame.Rect(300, 300, 200, 50)
    botao_jogo3 = pygame.Rect(300, 400, 200, 50)

    while True:
        screen.fill(WHITE)
        draw_text('Escolha um Jogo', font, BLACK, screen, screen_width // 2, 100)

        pygame.draw.rect(screen, GRAY, botao_jogo1)
        draw_text('Jogo 1', small_font, BLACK, screen, screen_width // 2, 225)

        pygame.draw.rect(screen, GRAY, botao_jogo2)
        draw_text('Jogo 2', small_font, BLACK, screen, screen_width // 2, 325)

        pygame.draw.rect(screen, GRAY, botao_jogo3)
        draw_text('Jogo 3', small_font, BLACK, screen, screen_width // 2, 425)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                x, y = event.pos
                if botao_jogo1.collidepoint(x, y):
                    iniciar_jogo("Pygame\\5.py")
                elif botao_jogo2.collidepoint(x, y):
                    iniciar_jogo("Pygame\\3.py")
                elif botao_jogo3.collidepoint(x, y):
                    iniciar_jogo("Pygame\\2.py")

        pygame.display.flip()

def iniciar_jogo(caminho_jogo):
    pygame.quit()
...
def main():
    ...
    pygame.quit()
    subprocess.Popen(["python", "Pygame\\\\4v3.py"])
    sys.exit()
